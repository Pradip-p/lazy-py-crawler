...
async function fetchAllCollections() {
    try {
        ...
        projectListSidebar.innerHTML = '<li class="collection-item active" id="btn-projects-view"># All Projects</li>';
        document.getElementById('btn-projects-view').onclick = () => showView('projects-view');
...
function showView(viewId) {
    document.getElementById('data-view').style.display = viewId === 'data-view' ? 'block' : 'none';
    document.getElementById('projects-view').style.display = viewId === 'projects-view' ? 'block' : 'none';
    ...
    projectForm.onsubmit = async (e) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                showToast('Error: ' + (err.detail || 'Failed to create project'), 'error');
            }
        } catch (err) {
...
async function loadProjects() {
    ...
    try {
        ...
        if (projects.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="5" class="text-center" style="padding: 3.5rem; color: #64748b;">No projects found. Create one to get started!</td></tr>';
            return;
        }
        ...
        projects.forEach(proj => {
            ...
            if (proj.status === 'Completed') statusClass = 'status-pill status-completed';
            else if (proj.status === 'Processing') statusClass = 'status-pill status-processing';
            else if (proj.status === 'Failed') statusClass = 'status-pill status-failed';
        ...
        tableBody.innerHTML = '<tr><td colspan="5" class="text-center" style="color: #ef4444; padding: 2rem;">Error loading projects. Please try again.</td></tr>';
    }
}
...
if (sidebarToggle && sidebar) {
    ...
    document.addEventListener('click', (e) => {
        if (window.innerWidth <= 768 && (e.target.classList.contains('collection-item') || e.target.id === 'btn-projects-view')) {
            sidebar.classList.remove('active');
        }
    });
}
